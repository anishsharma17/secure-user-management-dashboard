...
import { signupUser } from "./SignupForm.api";
...
type SignupPayload = Omit<FormState, "errors">;
...
export const SignupForm = () => {
  ...
  const authMutation = useMutation( (payload : SignupPayload) => {
    return signupUser(payload.email, payload.password);
  },
    {
      onSuccess: (response: AuthData) => {
        toast.success("sign up successfully!");
        setAuth(response);
        navigate(location.state || "/", { state: null });
      },
  ...
  const validation = (formValues: FormState) => {
    ...
    if (!(formValues.password.length && formValues.password.length > 5)) {
      setFormState((old) => {
        return { ...old, errors: { ...old.errors, password: "Please enter password greater than 5 character! " } };
      });
      return false;
    }
    return true;
          ...
          <form className="max-w-xl mx-auto" onSubmit={handleSubmit}>
            <div className="overflow-hidden sm:rounded-sm">
              <div className="px-4 py-5 sm:p-6">
                ...
                <a data-testid="forgetpasswordlink" href="/">
                  Already Registered
                </a>
              </p>
                </div>
